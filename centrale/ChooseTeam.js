...
export default function ChooseTeam({ navigation }) {
  ...
  const handleSubmit = () => {
    ...
    const apiUrl = `http://192.168.1.3:8080/team/teacherId/update/id/${teamId}`;
  ...
  const fetchTeamsData = async () => {
    ...
    const apiUrl = `http://192.168.1.3:8080/teams`;
      ...
      const teamsWithStudents = await Promise.all(
        data.map(async (team) => {
          ...
          for (let i = 1; i <= 3; i++) {
            ...
            const studentApiUrl = `http://192.168.1.3:8080/user/id/${studentId}`;
